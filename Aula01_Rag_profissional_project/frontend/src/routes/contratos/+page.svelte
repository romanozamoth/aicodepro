  ...
  async function performSearch(query: string) {
    ...
    try {
      ...
      if (err.code === 'ECONNABORTED' || err.message?.includes('timeout')) {
        error = 'A busca está demorando mais que o esperado. Isso é normal para buscas semânticas complexas. Por favor, aguarde ou tente uma consulta mais simples.';
      } else if (err.response?.status === 503) {
        error = 'Serviço temporariamente indisponível. Verifique se o backend está rodando.';
      } else {
